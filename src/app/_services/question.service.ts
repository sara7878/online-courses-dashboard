...
export class QuestionService {
  ...
  editQestion(id:number,updatedContent:Question): Observable<{data:Question[],status:boolean,error:any}> {
    console.log(id);
    console.log(updatedContent);
    return this.httpClient.post<{data:Question[],status:boolean,error:any}>(`${environment.baseUrl}questions/${id}`,updatedContent);
  }

  deleteQestion(id:number){
    console.log(id);
    return this.httpClient.delete<{data:Question,status:boolean,error:any}>(`${environment.baseUrl}questions/${id}`);
  }
